...
class CM_ND_AudioDopplerNode(bpy.types.Node):
    ...
    def get_sound(self):
        ...
        if sound_pre is not None:
            sound_o = sound_pre.join(sound_d)
        else:
            sound_o = sound_d

        if sound_post is not None:
            sound_o = sound_o.join(sound_post)

        if sound_o.specs[1] != cm.sound_channels:
            sound_o = sound_o.rechannel(cm.sound_channels)

        if self.orig_vol > 0:
            sound = sound.volume(self.orig_vol)
            sound_o = sound_o.mix(sound)
        return sound_o
